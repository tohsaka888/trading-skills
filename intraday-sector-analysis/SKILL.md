...
必须同时遵守硬约束：
1. 主分析宇宙固定为 A 股行业板块，不得混入概念板块、地域板块、指数类或通道类板块。
2. 只使用 `trading-mcp` 提供的数据，不联网，不补充外部新闻。
3. 结论必须以板块级数据为主，不做个股推荐，不展开成分股叙事。
4. 输出固定为“极简交易卡”，先给 Top5 和动作，再给证据。
